...
$dir = "../../Upl_Images/PressRelease/";
	...
	function uploadNews()
	{
		...
		mysql_query("INSERT INTO `update_detail`(`PublishDate`, `UpdateType`, `ContentType`, `Heading`, `Updates`, `UploadDate`, `UploadBy`, `IsActive`)VALUES('".$_POST['TXT_EventDt']."','pressrelease','image', '".$heading."','".$_FILES['FF_AlbumImage']['name']."',timestamp(now()),'".$_SESSION['username']."','1')")or die (mysql_error());
...
<!-- /.navbar -->
                ...
                <!-- ."main-bar -->
                    <div class="main-bar">
                        <h3><i class="fa fa-calendar"> </i> Press Release</h3>
                  </div>
                    <!-- /.main-bar -->
            </header>
                <!-- end header.head -->
                     ...
                     <div class="col-lg-6">
                      ...
                      <div class="col-lg-6">
                       <div class="box">
                         <header class="dark">
                        <div class="icons"><i class="fa fa-file-text-o fa-2x"></i></div>
                        <h5>Current Press Releases</h5>
                      </header>
                         <div class="body">
                         	
                          <?php
                          $res_NoticeList=mysql_query("SELECT * FROM `update_detail` WHERE `UpdateType`='pressrelease' AND  `IsActive`='1' ORDER BY `UploadDate` DESC");
                          $slid=1;
						 ...
						 <?php
                          $res_NoticeList=mysql_query("SELECT * FROM `update_detail` WHERE `UpdateType`='pressrelease' AND  `IsActive`='1'");
                          $slid=1;
                          while($row_NoticeLIst=mysql_fetch_array($res_NoticeList))
                          {
                          ?>
                          <tr>
                          <td width="70%" colspan="2" align="left"><img class="img img-thumbnail" src="<?php echo $dir."Thumbnail/".$row_NoticeLIst['Updates']; ?>" /></td>
