...
$res_totEventList = mysql_query("select * from `latest_news` 
								WHERE `LE_OI_OrganisationID`='".$_SESSION['OI_OrganisationID']."' order by LE_Slno DESC");
if(isset($_GET['del_eventsid']))
{
	...
	while($row_eventImg = mysql_fetch_array($res_eventImg))
	{
		...
		unlink("../../Upl_Images/EventImage/Thumbnail/".$row_eventImg['LE_C_Content']);
		unlink("../../Upl_Images/EventImage/Main/".$row_eventImg['LE_C_Content']);
		}
	}
...
<!-- /.navbar -->
                ...
                <!-- ."main-bar -->
                    <div class="main-bar">
                        <h3><i class="fa fa-calendar"></i> News Letter </h3>
                  </div>
                    <!-- /.main-bar -->
            </header>
                <!-- end header.head -->
                  ...
                  <div class="inner">
                    <div class="row">
                      <form id="form1" name="form1" method="post" action="">
                          ...
                          <div class="box">
                            ...
                            <div style="height: auto;" id="stripedTable" class="body in">
                              ...
                              <table border="0" cellpadding="5" cellspacing="0" class="table table-striped responsive-table col-lg-12">
                                <tr>
                                  <th width="59%">News Heading Name</th>
                                  <th width="13%">Date</th>
                                  <th width="13%">&nbsp;</th>
                                  <th width="15%">&nbsp;</th>
                                </tr>
						...
						$res_EventList = mysql_query("select * from latest_news WHERE `LE_OI_OrganisationID`='".$_SESSION['OI_OrganisationID']."' order by LE_Slno DESC LIMIT $lower, $upper");
	$i = 0;
	while($row_EventList=mysql_fetch_array($res_EventList))
	{
						...
						?>
                                <tr>
                                  ...
                                  <td><a href="UploadEventDescription.php?event_id=<?php echo $row_EventList['LE_Slno']; ?>" class="btn-sm btn-primary"><i class="fa fa-plus"></i>&nbsp;&nbsp;Add Content</a></td>
                                  <td>
						...
						?>
                        <tr>
                        <td align="center"><?php if(mysql_num_rows($res_EventList)==0){ echo "No Event Available Now";} ?></td>
                        </tr>
                              </table>
