...
$row_eventDesc = mysql_fetch_array(mysql_query("SELECT * FROM `menu_contents_detail` WHERE `LE_C_Slno` = '".$_GET['contid']."'"));
	...
	if(isset($_POST['BTN_UpdEventData']))
	{
			...
			else if($_POST['RBTN_EventType'] == "Image")
			{
				...
				upload_image($_FILES['IMG_event'], "../../Upl_Images/TextContent", $description, 600, 400, "yes");
			}
			if(!empty($description))
			{
			mysql_query("UPDATE `menu_contents_detail` 
						SET `LE_C_Content`='$description', `LE_C_Type`='".$_POST['RBTN_EventType']."',`LE_C_Status`='1'
						WHERE LE_C_Slno='".$_GET['contid']."'") or die(mysql_error());
			
			header("location:UploadMenuDescription.php");
			}
		}
?>
...
<!-- /.navbar -->
                ...
                <!-- ."main-bar -->
                    <div class="main-bar">
                        <h3><i class="fa fa-dashboard"></i> Update News Content</h3>
                  </div>
                    <!-- /.main-bar -->
            </header>
                <!-- end header.head -->
