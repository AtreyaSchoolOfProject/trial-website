...
$res_totEventList = mysql_query("select * from `menu_contents` 
								WHERE `LE_Slno`='".$_GET['mn']."' order by LE_Slno DESC");
if(isset($_GET['del_eventsid']))
{
	$res_eventImg = mysql_query("SELECT * FROM `menu_contents_detail` WHERE `LE_C_EventId` = '".$_GET['del_eventsid']."'");
	...
	mysql_query("DELETE FROM `menu_contents_detail` WHERE `LE_C_EventId` = '".$_GET['del_eventsid']."'");
	mysql_query("OPTIMIZE TABLE `menu_contents_detail`");
	mysql_query("DELETE FROM menu_contents WHERE LE_Slno = '".$_GET['del_eventsid']."'") or die(mysql_error());
	mysql_query("OPTIMIZE TABLE `menu_contents`");
...
<!-- /.navbar -->
                  ...
                  <div class="inner">
                    <div class="row">
                      <form id="form1" name="form1" method="post" action="">
						...
						$res_EventList = mysql_query("select * from menu_contents WHERE `MenuName`='".$_GET['mn']."' order by LE_Slno DESC LIMIT $lower, $upper");
	$i = 0;
	while($row_EventList=mysql_fetch_array($res_EventList))
	{
						...
						?>
                                <tr>
                                  <td><strong><?php echo $row_EventList['LE_Heading']; ?></strong></td>
                                  <td>
                                  <?php
								  	
									echo date('d M, Y', strtotime($row_EventList['LE_Date']));
								  ?>
                                  </td>
                                  <td><a href="UploadMenuDescription.php?event_id=<?php echo $row_EventList['LE_Slno']; ?>" class="btn-sm btn-primary"><i class="fa fa-plus"></i>&nbsp;&nbsp;Add Content</a></td>
                                  <td>
                                  
                                  <a href="MenuDetail.php?event_id=<?php echo $row_EventList['LE_Slno'] ?>" class="btn-sm btn-primary" style="text-align:left"><i class="fa fa-eye"></i>&nbsp;&nbsp;View</a>
						...
						?>
                        <tr>
                        <td align="center"><?php if(mysql_num_rows($res_EventList)==0){ echo "No Data Available Now";} ?></td>
                        </tr>
                              </table>
